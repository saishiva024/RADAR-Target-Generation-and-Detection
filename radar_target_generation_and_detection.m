...
Tchirp = (5.5 * 2.0 * radarMaxRange) / c;
slope = B / Tchirp;
...
RDM = RDM / max(RDM(:));
gridSize = 2 * (Td + Gd + 1) * 2 * (Tr + Gr + 1);
trainCells = gridSize - (Gr * Gd) - 1;
        ...
        for k = i - (Tr + Gr) : i + (Tr + Gr)
            for l = j - (Td + Gd) : j + (Td + Gd)
                if (abs(i - k) > Gr || abs(j - l) > Gd)
                    noiseLevel = noiseLevel + db2pow(RDM(k, l));
                end
            end
        end
        
        threshold = pow2db(noiseLevel / trainCells);
